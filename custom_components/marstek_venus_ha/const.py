"""Constants for the Marstek Venus HA integration."""

DOMAIN = "marstek_venus_ha"

PLATFORMS: list[str] = ["sensor", "switch"]

SIGNAL_DIAGNOSTICS_UPDATED = f"{DOMAIN}_diagnostics_updated"

# Configuration Keys
CONF_CT_MODE = "ct_mode"
CONF_GRID_POWER_SENSOR = "grid_power_sensor"
CONF_SMOOTHING_SECONDS = "smoothing_seconds"
CONF_ALLOW_CHARGING_FROM = "allow_charging_from"
CONF_ALLOW_CHARGING_TO = "allow_charging_to"
CONF_ALLOW_DISCHARGING_FROM = "allow_discharging_from"
CONF_ALLOW_DISCHARGING_TO = "allow_discharging_to"
CONF_MIN_SURPLUS = "min_surplus"
CONF_MIN_CONSUMPTION = "min_consumption"
CONF_MAX_LIMIT_BREACHES_BEFORE_ZEROING = "max_limit_breaches_before_zeroing"
CONF_MIN_SOC = "min_soc"
CONF_MAX_SOC = "max_soc"
CONF_MAX_DISCHARGE_POWER = "max_discharge_power"
CONF_MAX_CHARGE_POWER = "max_charge_power"
CONF_POWER_STAGE_OFFSET = "power_stage_offset"
CONF_PRIORITY_INTERVAL = "priority_interval"
CONF_WALLBOX_POWER_SENSOR = "wallbox_power_sensor"
CONF_WALLBOX_MAX_SURPLUS = "wallbox_max_surplus"
CONF_WALLBOX_CABLE_SENSOR = "wallbox_cable_sensor"
CONF_PV_POWER_SENSOR = "pv_power_sensor"
CONF_WALLBOX_POWER_STABILITY_THRESHOLD = "wallbox_power_stability_threshold"
CONF_WALLBOX_RESUME_CHECK_SECONDS = "wallbox_resume_check_seconds"
CONF_WALLBOX_START_DELAY_SECONDS = "wallbox_start_delay_seconds"
CONF_WALLBOX_RETRY_MINUTES = "wallbox_retry_minutes"
CONF_WALLBOX_STABILITY_MIN_POWER_GAP = "wallbox_stability_min_power_gap"
CONF_WALLBOX_STABILITY_MIN_GAP_DURATION_SECONDS = "wallbox_stability_min_gap_duration_seconds"
CONF_COORDINATOR_UPDATE_INTERVAL_SECONDS = "coordinator_update_interval_seconds"
CONF_SERVICE_CALL_CACHE_SECONDS = "service_call_cache_seconds"

CONF_PID_ENABLED = "pid_enabled"
CONF_PID_KP = "pid_kp"
CONF_PID_KI = "pid_ki"
CONF_PID_KD = "pid_kd"
CONF_PID_FEEDFORWARD_ENABLED = "pid_feedforward_enabled"
CONF_PID_FEEDFORWARD_GAIN = "pid_feedforward_gain"
# New Battery Configuration
CONF_BATTERY_COUNT = "battery_count"
DEFAULT_BATTERY_COUNT = 1

# Default values
DEFAULT_CT_MODE = False
DEFAULT_SMOOTHING_SECONDS = 0
DEFAULT_ALLOW_CHARGING_FROM = "00:00"
DEFAULT_ALLOW_CHARGING_TO = "24:00"
DEFAULT_ALLOW_DISCHARGING_FROM = "00:00"
DEFAULT_ALLOW_DISCHARGING_TO = "24:00"
DEFAULT_MIN_SURPLUS = 200
DEFAULT_MIN_CONSUMPTION = 200
DEFAULT_MAX_LIMIT_BREACHES_BEFORE_ZEROING = 5
DEFAULT_MIN_SOC = 12
DEFAULT_MAX_SOC = 100
DEFAULT_MAX_DISCHARGE_POWER = 2500
DEFAULT_MAX_CHARGE_POWER = 2500
DEFAULT_POWER_STAGE_OFFSET = 500
DEFAULT_PRIORITY_INTERVAL = 15
# Charge level power caps (5 levels)
CONF_CHARGE_POWER_LEVEL_1 = "charge_power_level_1"
CONF_CHARGE_POWER_LEVEL_2 = "charge_power_level_2"
CONF_CHARGE_POWER_LEVEL_3 = "charge_power_level_3"
CONF_CHARGE_POWER_LEVEL_4 = "charge_power_level_4"
CONF_CHARGE_POWER_LEVEL_5 = "charge_power_level_5"

# Discharge level power caps (5 levels)
CONF_DISCHARGE_POWER_LEVEL_1 = "discharge_power_level_1"
CONF_DISCHARGE_POWER_LEVEL_2 = "discharge_power_level_2"
CONF_DISCHARGE_POWER_LEVEL_3 = "discharge_power_level_3"
CONF_DISCHARGE_POWER_LEVEL_4 = "discharge_power_level_4"
CONF_DISCHARGE_POWER_LEVEL_5 = "discharge_power_level_5"

# Defaults for level caps (examples provided by user)
DEFAULT_CHARGE_POWER_LEVEL_1 = 1500
DEFAULT_CHARGE_POWER_LEVEL_2 = 1800
DEFAULT_CHARGE_POWER_LEVEL_3 = 2000
DEFAULT_CHARGE_POWER_LEVEL_4 = 2200
DEFAULT_CHARGE_POWER_LEVEL_5 = 2400

DEFAULT_DISCHARGE_POWER_LEVEL_1 = 1500
DEFAULT_DISCHARGE_POWER_LEVEL_2 = 1800
DEFAULT_DISCHARGE_POWER_LEVEL_3 = 2000
DEFAULT_DISCHARGE_POWER_LEVEL_4 = 2200
DEFAULT_DISCHARGE_POWER_LEVEL_5 = 2400
DEFAULT_WALLBOX_MAX_SURPLUS = 1500
DEFAULT_WALLBOX_POWER_STABILITY_THRESHOLD = 100
DEFAULT_WALLBOX_RESUME_CHECK_SECONDS = 300
DEFAULT_WALLBOX_START_DELAY_SECONDS = 120
DEFAULT_WALLBOX_RETRY_MINUTES = 60
DEFAULT_WALLBOX_STABILITY_MIN_POWER_GAP = 500
DEFAULT_WALLBOX_STABILITY_MIN_GAP_DURATION_SECONDS = 30
DEFAULT_COORDINATOR_UPDATE_INTERVAL_SECONDS = 2
DEFAULT_SERVICE_CALL_CACHE_SECONDS = 30

DEFAULT_PID_ENABLED = False
DEFAULT_PID_KP = 0.5
DEFAULT_PID_KI = 0.02
DEFAULT_PID_KD = 0.0
DEFAULT_PID_FEEDFORWARD_ENABLED = False
DEFAULT_PID_FEEDFORWARD_GAIN = 0.7
